...
void main()
{
    ...
    if (choice == -1)
    {
        printf("User exitted Successfully\n");
        close(sd);
        return;
    }
    ...
    while (1) // we want to enter the while 1 only when login is successful
    {
        
        if (choice == 1) // normal user
        {
            int option;
            printf("Press 3 to issue book or 4 to return book or -1 to exit\n");
            scanf("%d", &option);
        ...
        {
            int option;
            printf("Press 1 to add user, 2 to add book, 5 to get book details and 6 to modify book details and -1 to exit\n");
            scanf("%d", &option);
            write(sd, &option, sizeof(option));
            if (option == 1)
            {
